"""
ZeroDBSensor -- Polls ZeroDB events and returns Dagster RunRequests.
...
class ZeroDBEvent:
    ...
    @classmethod
    def from_dict(cls, d):
        """Create a ZeroDBEvent from a dict."""
        return cls(
    ...
    def to_run_request(self):
        """Convert to a dict suitable for Dagster RunRequest run_config.

        Returns:
            dict with event data for run configuration.
        """
        return {
            "run_key": self.event_id,
            "run_config": {
                "ops": {
                    "zerodb_event": {
                        "config": self.to_dict(),
...
class ZeroDBSensor:
    """Polls ZeroDB events and triggers handlers (Dagster sensor pattern).
    ...
    def process_webhook(self, payload):
        ...
        Args:
            payload: Dict from webhook request body.
